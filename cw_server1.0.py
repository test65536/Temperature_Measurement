...
while True:
    ...
    db.commit()
    sheet = cursor.fetchall()  # 加一个for循环遍历所有设备地址以及是否为新设备（未加！）
    addr = (sheet[0])[1]
    print(sheet)
    print("设备总数:",len((sheet)))
    # 发送数据crc16校验
    send_crc = crc16()
    ...
    refer_zigbee = [0xFC,0x0C,0x03,0x01,0xFF,0xFF,addr,0x03,0x00,0x02,0x00,0x02,int(send_crc_h,16),int(send_crc_l,16)]
