...
class crc16:
    ...
    def calcrc(self, array):
        ...
        if crchi == array[lenarray - 2] and crclo == array[lenarray - 1]:
            ...
            return 1
# 2018.7.24 19:00 未写入休眠时间和遍历休眠时间
...
while True:
    ...
    for tat in time_addr_table:
        ...
        if new_old == 'N':
            ...
            while True:
                ...
                while True:
                    ...
                    r = tuple(ser.read(20))  # 接收模块地址数据，read为字节长度
                    print(r)
                    if len(r) == 0 or (r[0] == 247 and r[1] == 255):
                        print("地址%d读取失败" % addr)
                        i = i + 1
                    elif addr == r[4]:
                        print("地址%d写入成功" % addr)
        ...
        # 读取温度电压数据
            ...
            refer_zigbee = [0xFC, 0x0C, 0x03, 0x01, 0xFF, 0xFF, addr, 0x03, 0x00, 0x02, 0x00, 0x02, int(send_crc_h, 16),
                            int(send_crc_l, 16)]
            # 如果接收数据失败，重复执行5次
            refer_flag = 0
            while True:
                ...
                cal_received_crc_l = cal_received_crc_raw[4:]  # 计算接收数据crc校验低位，原始crc的高位,字符串
